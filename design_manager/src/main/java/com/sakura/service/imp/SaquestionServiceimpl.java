...
import com.sakura.mapper.SaquestionMapper;
...
@Service
public class SaquestionServiceimpl implements SaquestionService {
    @Autowired
    ...
    public EUDataGridResult gopage(Integer pageNo, Integer pageSize,String qname, String cname) {
        ...
        SaquestionExample example = new SaquestionExample();
        SaquestionExample.Criteria criteria = example.createCriteria();
        if (!"".equals(qname)&&qname!=null)
            criteria.andNameLike("%"+qname+"%");
        if (!"".equals(cname)&&cname!=null)
            criteria.andCnameLike("%"+cname+"%");
        PageHelper.startPage(pageNo,pageSize);
        List<Saquestion> list = saquestionMapper.selectByExample(example);
    ...
    public void addmQustion(Saquestion saquestion) {
        if (saquestion.getId()!=null){
            saquestionMapper.updateByPrimaryKeySelective(saquestion);
        }else {
            saquestion.setAdddate(new Date());
            saquestionMapper.insert(saquestion);
        }
    }

    @Override
    public Saquestion getquebyid(Long id) {
        Saquestion saquestion = saquestionMapper.selectByPrimaryKey(id);
    ...
    public String delete(String ids) {
        ...
        for (String s : split) {
            if (!"".equals(s))
            saquestionMapper.deleteByPrimaryKey(Long.parseLong(s));
    ...
    @Override
    public void addque(PaperQue<Saquestion> paperQue) {
        List<Saquestion> list = paperQue.getList();
        for (Saquestion mquestion : list) {
